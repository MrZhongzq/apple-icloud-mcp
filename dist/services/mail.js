...
export const getSmtpTransporter = (account) => {
...
export const listEmails = async (account, folder = 'INBOX', limit = 10) => {
    if (!account.allowMailRead)
        throw new Error('Mail read permission denied.');
    let connection = null;
    try {
...
export const readEmail = async (account, uid, folder = 'INBOX') => {
    if (!account.allowMailRead)
        throw new Error('Mail read permission denied.');
    let connection = null;
    try {
...
export const sendEmail = async (account, to, subject, text) => {
    if (!account.allowMailWrite)
...
export const deleteEmail = async (account, uid, folder = 'INBOX') => {
    if (!account.allowMailWrite)
        throw new Error('Mail write permission denied.');
    let connection = null;
    try {
