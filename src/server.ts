...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === 'apple_login') {
      const res = await login(args.email as string);
      return { content: [{ type: 'text', text: JSON.stringify(res, null, 2) }] };
    }

    if (name === 'work_done') {
      const success = logout(args.session_id as string);
    ...
    switch (name) {
      // Mail
      case 'mail_list':
        result = await listEmails(session.account, args.folder as string, args.limit as number);
        break;
      case 'mail_read':
        result = await readEmail(session.account, args.uid as number, args.folder as string);
        break;
      case 'mail_send':
        result = await sendEmail(session.account, args.to as string, args.subject as string, args.text as string);
        break;
      case 'mail_delete':
        result = await deleteEmail(session.account, args.uid as number, args.folder as string);
        break;
      
      // Calendar
      case 'calendar_list_events':
        result = await listEvents(session.account, args.start as string, args.end as string);
        break;
      case 'calendar_create_event':
        result = await createEvent(session.account, args.ical_string as string, args.filename as string);
        break;
      case 'calendar_update_event':
        result = await updateEvent(session.account, args.calendar_url as string, args.event_url as string, args.ical_string as string, args.etag as string);
        break;
      case 'calendar_delete_event':
        result = await deleteEvent(session.account, args.event_url as string, args.etag as string);
        break;
